...
public class ServicesSaleReportRevenueMonthByProduct {
  ...
  /** method for retrive. */
  public ResponseEntity<ArrayList<Double>> getReport(Integer year, Integer month, Long idProduct) {
    YearMonth yearMonthObject = YearMonth.of(year, month);
    int daysInMonth = yearMonthObject.lengthOfMonth();
    ArrayList<Double> arrayDays = new ArrayList<>();
    for (int day = 1; day <= daysInMonth; day++) {
      String date = year + "-" + month + "-" + day;
      Double sumDay = repositorySaleProduct.getRevenuePerDayByProduct(date, idProduct);
