  ...
  public ResponseEntity<List<DtoAlcaldiaRecursosMunicipales>> filter(
    ...
    if (query.equals("")) {
      ...
      String fullQuery = "SELECT DISTINCT alcaldia_recursos_municipales.* FROM alcaldia_recursos_municipales "
          ...
          + "NOT IN (SELECT id FROM alcaldia_recursos_municipales WHERE name = (SELECT alcaldia_recursos_municipales.name FROM alcaldia_recursos_municipales"
          + "INNER JOIN alcaldia_recursos_municipales_grupo_para_reportes "
