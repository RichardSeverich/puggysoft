...
const GeneratePdf = async (data) => {
  ...
  doc.setFontSize(15);
  doc.text(i18n.HistoryPorEstudianteCursoTable.tableTitlePdf, 50, 29);

  doc.setFontSize(8);
  doc.text(i18n.HistoryPorEstudianteCursoTable.fecha, pageWidth - 83, 29);
  doc.text(timeNow(), pageWidth - 65, 29);

  const lineVerticalX = (pageWidth - 35) / 3;
  doc.line(lineVerticalX, 40, lineVerticalX, 65);

  doc.setFontSize(10);
  doc.text(
    `${i18n.HistoryPorEstudianteCursoTable.name}: ${data.estudiante.name} ${data.estudiante.lastName}`,
    (lineVerticalX / 2) + 15,
    51,
    {
      align: "center",
  ...
  doc.text(
    `${i18n.HistoryPorEstudianteCursoTable.ciColumn} ${data.estudiante.dni}`,
    (lineVerticalX / 2) + 15,
    59,
    {
      align: "center",
